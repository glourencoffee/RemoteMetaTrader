...
class ErrorCode(Enum):
    USER_UNKNOWN_COMMAND         = -1
    USER_MISSING_REQUIRED_PARAM  = -2
    USER_INVALID_PARAMETER_TYPE  = -3
    USER_INVALID_PARAMETER_VALUE = -4
...
#TODO: should this be actually thrown?
class Again(RequestError):
    pass
...
class UnknownServerCommand(RequestError):
    def __init__(self, command: str) -> None:
        super().__init__("Server does not recognize command '{}'".format(command))

class MissingRequiredParameter(RequestError):
    def __init__(self, command: str, param: str) -> None:
        super().__init__("Missing required parameter '{}' for command '{}'".format(param, command))

class InvalidParameterType(RequestError):
    def __init__(self, command: str, param: str) -> None:
        super().__init__("Invalid type for parameter '{}' for command '{}'".format(param, command))

class InvalidParameterValue(RequestError):
    def __init__(self, command: str, param: str) -> None:
        super().__init__("Invalid value for parameter '{}' for command '{}'".format(param, command))

class ExecutionError(RMTError):
    def __init__(self, command: str, error_code: ErrorCode) -> None:
        super().__init__("Execution of command '{}' failed with error: {}".format(command, error_code))